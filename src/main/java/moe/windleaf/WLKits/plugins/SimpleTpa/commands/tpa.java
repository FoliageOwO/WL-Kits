...
public class tpa implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!(sender instanceof Player)) {
            ...
            if (args.length < 1) {
                s.send(sender, "&a使用 &6/tpahelp &a查看帮助!");
                return false;
            } else {
                Player player = Bukkit.getPlayer(args[0]);
                if (player == sender) {
                    s.send(sender, "&c你不能给自己发送传送请求!");
                    return false;
                } else {
                    if (player == null) {
                        s.send(sender, "&c玩家 &6" + args[0] + " &c不存在!");
                    } else {
                        SimpleTpa.tpaLogs.put(player, (Player) sender);

                        s.send(player, "&a你有一个待接受的传送请求:");
                        s.send(player, String.format("&a玩家 &6%s &a想传送到你这里,", sender.getName()));
                        s.send(player, "&a接受请求: &2/tpaccept&a, 拒绝请求: &4/tpadeny&a.");

                        s.send(sender, String.format("&a已向玩家 &6%s &a发送一个传送请求, 等待对方接受!", player.getName()));
                        s.send(sender, "&a想要取消这个传送请求, 你可以输入 &6/tpacancel&a.");
                    }
                    return true;
                }
            }
